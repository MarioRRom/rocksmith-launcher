...
public:
    ...
    // Enables the patch on the profile and persists the toggle. Returns false
    // (with error filled) when the profile or patch is unknown or incompatible.
    bool Enable(const std::string &profileId, const std::string &patchId, std::string &error);
    // Disables the patch on the profile and persists the toggle.
    bool Disable(const std::string &profileId, const std::string &patchId, std::string &error);
